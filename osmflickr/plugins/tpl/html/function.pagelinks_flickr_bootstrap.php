 ...
 *  */
function jtpl_function_html_pagelinks_flickr_bootstrap($tpl, $action, $actionParams, $pagesTotal, $page, $paramName = 'page', $displayProperties = array())
{
    $page = intval($page);
    if ($page <= 1)
        $page = 1;
    ...
    if ($pagesTotal > 1) {
         ...
         */
        for($numpage = 1; $numpage <= $pagesTotal; $numpage++) {
          if ($page == $numpage) {
            $pages[$numpage] = '<li class="pagelinks-current active"><a href="#">'.$numpage.'</a></li>';
            $prevBound = $numpage - 1;
            $nextBound = $numpage + 1;
            ...
            $urlaction->params[$paramName] = $numpage;
            $url = $jUrlEngine->create($urlaction);
            $pages[$numpage] = '<li><a href="'.$url->toString(true).'">'.$numpage.'</a></li>';
          }
        }
        ...
        $urlaction->params[$paramName] = $pagesTotal;
    ...
    else echo '<div class="pagination"><ul class="pagelinks"><li class="pagelinks-current active"><a href="#">1</a></li></ul></div>';
}
